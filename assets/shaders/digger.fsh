...
void main (void)
{
	...
	mediump float gradpower = clamp( length(dif), 0.0, 1.0);
						
	// alphamul
	mediump vec2 tv = (texCoord-vec2(0.5, 0.5))*2.0;
	mediump float alphamul = clamp( 1.0-( tv.x*tv.x + tv.y*tv.y ), 0.0, 1.0);

	gl_FragColor = vec4(texture2D(tex1,tex1Coord).rgb,gradpower*vertexColor.a*alphamul);
}
