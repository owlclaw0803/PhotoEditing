...
void main (void)
{
	...
	t0 = clamp( 0.5 - t0*t0, 0.0, 1.0 );
	//t0 = clamp( 0.55 - sqrt(t0), 0.0, 1.0 );
	
	mediump float t1 = dot(xy1, xy1);
	t1 = clamp( 0.5 - t1*t1, 0.0, 1.0 );
	//t1 = clamp( 0.55 - sqrt(t1), 0.0, 1.0 );
	
	mediump float t2 = dot(xy2, xy2);
	t2 = clamp( 0.5 - t2*t2, 0.0, 1.0 );
	//t2 = clamp( 0.55 - sqrt(t2), 0.0, 1.0 );
	...
	gl_FragColor = vec4(tcol,alphamul*vertexColor.a);
}
