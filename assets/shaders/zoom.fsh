...
/*    
    vec3 blurred = texture2DLodEXT( backbuffer, mix(tex1Coord, vec2(0.5), 0.1), amount-1.0).rgb;
...
*/	
	...
	c = mix(c, texture2D( backbuffer, mix(tex1Coord, vec2(0.5), a)).rgb, 0.25);
	c = mix(c, texture2D( backbuffer, mix(tex1Coord, vec2(0.5), a*2.0)).rgb, 0.25);
	c = mix(c, texture2D( backbuffer, mix(tex1Coord, vec2(0.5), a*3.0)).rgb, 0.25);
	c = mix(c, texture2D( backbuffer, mix(tex1Coord, vec2(0.5), a*4.0)).rgb, 0.25);
	c = mix(c, texture2D( backbuffer, mix(tex1Coord, vec2(0.5), a*5.0)).rgb, 0.25);
